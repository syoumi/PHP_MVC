...
$config = [
    ...
    'core_classes'=> ['Controller', 'Loader'],
    'models' => ['Soccer']
];
...
function create_controller($controller_name) {
    ...
    if (!file_exists($controller_filename)) { throw new Exception("Controller file does not exist!"); }
...
function call_controller_method($controller, $method_name, $parameters) {
    ...
    if (! $reflectionObject->hasMethod($method_name)) { throw new Exception("Method name does not exist!"); }
    $reflectionMethod = $reflectionObject->getMethod($method_name);
    if ($reflectionMethod->getNumberOfParameters()!=count($parameters)){ throw new Exception("Parameters not supplied !"); }
